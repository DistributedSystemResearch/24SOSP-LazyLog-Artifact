...
namespace OpenMsgCpp {
    ...
    void producer::setLatLog(std::string input) {
        latencyLog = input;
    }

    void producer::run() {
        auto msg = "Runnig producer with config: messageSize: " +
            load.getConfig("messageSize") + ", producerRate: " + load.getConfig("producerRate") + " for " +
            load.getConfig("testDurationMinutes") + "m...";
        log_info(msg);
        ...
        log_info("Producer finish with " + std::to_string(numEntries) + " entries appended, tput: " + 
                 std::to_string(tput) + "entries/s");
        if (!latencyLog.empty()) {
