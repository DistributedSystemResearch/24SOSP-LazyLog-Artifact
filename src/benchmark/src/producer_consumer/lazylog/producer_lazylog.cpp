...
namespace OpenMsgCpp {
    ...
    void producerLazylog::initClient(Properties prop) {
        llClient.Initialize(prop);
    }


    int producerLazylog::produce(
            std::atomic<int> &tail, std::vector<std::chrono::high_resolution_clock::time_point> &writeTime) {
    ...
    int producerLazylog::produceSync(
            std::atomic<int> *tail, std::vector<std::chrono::high_resolution_clock::time_point> *writeTime) {
        llClient.AppendEntryAll(message);
        if (writeTime)
            writeTime->push_back(std::chrono::high_resolution_clock::now());
