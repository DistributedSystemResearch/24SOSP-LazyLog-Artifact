...
namespace OpenMsgCpp {
    ...
    void log_info(std::string msg) {
        ...
        auto microseconds =
            std::chrono::time_point_cast<std::chrono::microseconds>(currentTime).time_since_epoch().count();
    ...
    void sleep_for_nanoseconds_erpc(std::chrono::nanoseconds ns, producerLazylog *producer) {
        ...
        while (std::chrono::high_resolution_clock::now() < end) {producer->rpcOnce();}
    }
    ...
    void RealtimeTput::record(uint64_t num) {
